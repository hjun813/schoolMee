...
1.  **실행**: `docker-compose up --build -d` 실행
2.  **접속**: `http://localhost:5173` 접속
3.  **데이터 생성 (2가지 방법)**:
    -   **방법 A (Quick Start)**: 메인 화면의 **[데모 데이터 생성]** 버튼 클릭 (샘플 학교/학생/스토리 즉시 구축)
    -   **방법 B (직접 테스트)**: 온보딩 절차를 따라 직접 등록. 루트의 `photo` 폴더에 있는 샘플 사진(메시, 손흥민 등)을 사용하여 학생 등록 및 AI 매칭 로직을 실제와 동일하게 검증 가능
